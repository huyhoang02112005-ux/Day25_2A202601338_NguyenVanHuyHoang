    ...
    for breaker in gateway.breakers.values():
        ...
        for entry in breaker.transition_log:
            if entry["to"] == "open":
                ...
                recovery_times.append((float(entry["ts"]) - last_open_ts) * 1000.0)
                last_open_ts = None
...
def run_scenario(config: LabConfig, queries: list[str], scenario: ScenarioConfig) -> RunMetrics:
    ...
    metrics.circuit_open_count = sum(
        1 for b in gateway.breakers.values() for entry in b.transition_log if entry["to"] == "open"
    )
...
def run_simulation(config: LabConfig, queries: list[str]) -> RunMetrics:
    ...
    for scenario in config.scenarios:
        ...
        passed = result.successful_requests > 0 and (result.successful_requests / result.total_requests) >= 0.8
